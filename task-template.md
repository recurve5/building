...
- **Dependency gate (hard).** A task whose dependencies are not all marked `done` cannot begin execution. The agent does not attempt to resolve the dependency — it reports the block and stops. This is enforced by the Peer Reviewer (see `peer-reviewer.md`, Orchestration) and is a hard gate, not a suggestion.

...
- **Milestone directories.** Each milestone's tasks live in that milestone's `tasks/` subdirectory (e.g., `m1-nacre-docx-ingestion/tasks/001-setup-parser.md`). Task filenames do not need milestone prefixes because the directory structure provides namespacing. See `orchestrator.md` Stage 0 for the full directory structure convention.

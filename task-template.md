...
- **Milestone directories.** Each milestone's tasks live in that milestone's `tasks/` subdirectory (e.g., `m1-recipebook-ingredient-parser/tasks/001-setup-parser.md`). Task filenames do not need milestone prefixes because the directory structure provides namespacing. See `orchestrator.md` Stage 0 for the full directory structure convention.
...
- When a task is done, add a "Completed" section with the date, any deviations from the plan, and — critically — an **Insight/Implication** note. What did building this task reveal that wasn't in the spec? What does that mean for upcoming tasks? Example: "**Insight:** the CartService needed a special case for items added during an active sale — the price lookup hits a race condition when a promotion expires mid-session, producing ambiguity about which price applies. **Implication:** Task 007 (bulk discount rules) will need a similar edge case for promotions that span multiple items. Add it to that task's acceptance criteria now."

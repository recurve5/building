...
describe('writeEvent', () => {
  ...
  // EV-001: writeEvent writes valid JSON to <runDir>/events/<eventType>.json
  it('EV-001: writes valid JSON to the correct path', () => {
    writeEvent(runDir, 'test_event', { key: 'value' });
    const filePath = join(runDir, 'events', 'test_event.json');
  ...
  it('EV-003: enriches payload with event field', () => {
    ...
    const filePath = join(runDir, 'events', 'my_event.json');
  ...
  it('EV-004: enriches payload with ISO 8601 timestamp', () => {
    ...
    const filePath = join(runDir, 'events', 'ts_event.json');
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
    ...
    // Verify timestamp is within the test window
  ...
  it('EV-005: writes session_boundary event', () => {
    writeEvent(runDir, 'session_boundary', {
      session_id: 'abc-123',
      reason: 'start',
    });
    const filePath = join(runDir, 'events', 'session_boundary.json');
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
  ...
  it('EV-006: writes handoff_written event', () => {
    writeEvent(runDir, 'handoff_written', {
      handoff_path: '/tmp/handoff.md',
      milestone: 'm3',
    });
    const filePath = join(runDir, 'events', 'handoff_written.json');
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(parsed.event).toBe('handoff_written');
    expect(parsed.handoff_path).toBe('/tmp/handoff.md');
    expect(parsed.milestone).toBe('m3');
    expect(parsed.timestamp).toBeDefined();
  });

  // EV-007: writeEvent for session_resumed event type
  it('EV-007: writes session_resumed event', () => {
    writeEvent(runDir, 'session_resumed', {
      previous_session: 'old-session-id',
      resumed_at: 'task-004',
    });
    const filePath = join(runDir, 'events', 'session_resumed.json');
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(parsed.event).toBe('session_resumed');
    expect(parsed.previous_session).toBe('old-session-id');
    expect(parsed.resumed_at).toBe('task-004');
    expect(parsed.timestamp).toBeDefined();
  });

  // EV-008: writeEvent overwrites existing event file with same type
  it('EV-008: overwrites existing event file with same type', () => {
    writeEvent(runDir, 'overwrite_test', { version: 1 });
    writeEvent(runDir, 'overwrite_test', { version: 2 });
    const filePath = join(runDir, 'events', 'overwrite_test.json');
    const parsed = JSON.parse(readFileSync(filePath, 'utf-8'));
    expect(parsed.version).toBe(2);
    expect(parsed.event).toBe('overwrite_test');
  });
});
